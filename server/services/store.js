...
export async function getCategories(
  ...
  const [rows] = await connection.query(
    'SELECT data FROM categories ORDER BY sort_order ASC, name ASC',
  );
...
export async function getCoupons(
  ...
  const [rows] = await connection.query(
    'SELECT data FROM coupons ORDER BY code',
  );
...
export function sanitizeProduct(product) {
  ...
  return {
    ...safe,
    variants: (safe.variants || []).map(
      ({ costPrice: variantCost, ...variant }) =>
        variant,
    ),
  };
}
  ...
  try {
    await connection.execute(
      ...
      [
        ...
        String(order.shippingAreaId || ''),
        String(order.shippingAreaName || ''),
        String(order.paymentMethodId || ''),
        String(order.paymentMethodName || ''),
