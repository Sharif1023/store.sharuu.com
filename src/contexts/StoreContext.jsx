...
export function StoreProvider({ children }) {
  ...
  const applyTheme = settings => {
    ...
    const browserLogo = String(
      settings.browserLogo || settings.logo || '/logo.png'
    ).trim() || '/logo.png';
    ...
    if (socialImage) socialImage.setAttribute('content', browserLogo);

    const socialTitle = document.querySelector('meta[property="og:title"]');
    if (socialTitle) {
      socialTitle.setAttribute(
        'content',
        settings.storeName || 'Sharuu Universal Store'
      );
    }
  };
